...
the number of rooted unlabelled trees is unknown which stands testament to the difficulty in understanding tree automorphisms.  
An intriguing 
...
provides further motivation, if we needed any, to investigate automorphism groups of trees \cite{macarthur2006}.  We exploit a geometric interpretation of 
...
$f:\T \rightarrow \mathbb{R}$ and describe how these may be manipulated to calculate properties of trees such as order, path length and 
...
composition of maps forms a group denoted $\Aut(t)$.  The order of the automorphism group is 
...
\end{align}
The notation $t!$ is taken from Kreimer \cite{kreimer} because $t!$ generalises the factorial of a number.
...
\end{equation}
where each factor $A_i$ is isomorphic to a symmetric group and each $B_i$ is isomorphic to the wreath product of a group $G \in \mathcal{W}$.  In \cite{macarthur} MacArthur, 
...
\end{equation}
where each $H_i$ is generated by $S_i$.  This, \emph{geometric decomposition} is shown \cite{macarthur} to be unique and irreducible (each $H_i$ cannot be written as the direct product of support-disjoint subgroups) hence the geometric decomposition is well defined. We call each $H_i$ in the geometric decomposition of $\Aut(t)$ a \emph{geometric factor}. A symmetric subtree is the induced subtree of $t$ on the support of a geometric factor.
\begin{ex}
\begin{itemize}
...
\]
This begs the question: does the order of either the elementary or the complex subgroup dominate the other?  MacArthur \cite{macarthur} made the following additional conjecture:
...
\]
\begin{ex}
 Let $\delta_{i,j}$ be the usual Kronecker delta. The following is a list of possible sequences $f$ and the tree parameter 
...
\[
 \alpha(\hat{t}_k) = 1
\]
i.e. there is only one isomorphism class of random recursive trees on $k$ vertices isomorphic to $\hat{t}_k$.  Fik $k \geq 3$ 
